...
SELECT
	first_name,					--Ellen, ~
	upper(first_name),			--ELLEN, ~~
	lower(first_name)			--ellen, ~~
FROM employees;

SELECT
	'abc',						--abc
	initcap('abc'),				--Abc : 첫문자를 대문자로
	initcap('aBC')				--Abc : 나머지 문자는 소문자로
FROM dual;

--요구사항) 이름(frist_name)에 'an'이 포함된 직원 > 대소문자 구분 없이
SELECT
	first_name
FROM employees
-- WHERE first_name LIKE '%an%';		-- Anna(x)
...
문자열 추출 함수
- substr() >  subString()
- varchar2 substr(컬럼, 시작 위치, 가져올 문자 개수)
- varchar2 substr(컬럼, 시작 위치)
*/
...
SELECT name, LENGTH(name) AS leng FROM tblcountry WHERE leng > 3;		--RA-00904: "LENG": invalid identifier
SELECT name, length(name) FROM tblcountry ORDER BY leng asc();
...
- number instr(컬럼, 검색어, 시작위치, -1)   // lastIndex
*/
...
SELECT
	...
	instr('안녕하세요. 홍길동님. 홍길동님', '홍길동' ,instr('안녕하세요. 홍길동님. 홍길동님', '홍길동') + LENGTH('홍길동')) AS r5,
...
- decode()
- replace()와  유사
- vahchar2 decode(컬럼, 찾을 문자열, 바꿀 문자열, [찾을 문자열, 바꿀 문자열] x N)
*/
...
SELECT
	gender, 
	CASE
		WHEN main gender = 'm'  THEN '남자'
		WHEN main gender = 'f'  THEN '여자'
	END AS g1,

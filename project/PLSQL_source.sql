...
BEGIN
    ...
    LOOP
        IF rec."과목상태" = '종료' THEN
            dbms_output.put_line('과정명 : ' || rec."과정명" || '(' || rec."과목상태" || ')');
            dbms_output.put_line('(' || rec."과정 시작일" || ' ~ ' || rec."과정 종료일" || ')');
...
BEGIN
   ...
   LOOP
      ...
      DBMS_OUTPUT.PUT_LINE('이전근무처 : ' || rec."이전 근무처" || '(' || TO_CHAR(rec."근무 시작일", 'RRRR/MM/DD') || ' ~ ' || TO_CHAR(rec."근무 종료일", 'RRRR/MM/DD') || ')');
      DBMS_OUTPUT.PUT_LINE('취업기업명 : ' || rec."취업기업명" || '(' || TO_CHAR(rec."취업일", 'RRRR/MM/DD') || ' ~ )');
      dbms_output.put_line('취업희망분야 : ' || rec."취업 희망 분야" || '  |  희망지역 : ' || rec."희망지역" || '  |  희망급여 : ' || rec."희망급여");      
      DBMS_OUTPUT.PUT_LINE('------------------------------------------------------------------------------------------');
   END LOOP;

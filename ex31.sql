--ex31.sql
--근태 상황(출석)
CREATE TABLE tblDate(seq NUMBER PRIMARY KEY, state varchar2(30) NOT NULL, regdate DATE NOT null);

--insert into tblDate (attend_seq, student_seq, attendState_seq, attendDate, attendStartTime, attendEndDate) 
-- values (1, '정상', '2023-09-01');
...
-- 근태 조회 > 9월 근태기록 열람 > 결석한 날짜도 포함 + 공휴일 포함 > 빠진 날짜 메꾸기
--1. ANSI-SQL
--2. PL-SQL -> for문
--3. Java -> for문
...
CREATE OR REPLACE VIEW vwDate AS --뷰 테이블 생성(?) 반환값 생성ㄴㄴ -> 프로시저 사용 OR cursor사용
SELECT to_date('20230901', 'yyyy-mm-dd') + LEVEL - 1 AS regdate FROM dual
CONNECT BY LEVEL <= (to_date('20230930', 'yyyy-mm-dd') - to_date('20230901', 'yyyy-mm-dd') + 1);
...
--INNER JOIN tblDate t ON v.regdate = t.regdate;
...
--공휴일 처리
CREATE TABLE tblHoliday(seq NUMBER PRIMARY KEY, regdate DATE NOT NULL, name varchar2(30) NOT null);
